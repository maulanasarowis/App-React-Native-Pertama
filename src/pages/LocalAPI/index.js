      ...
      <TouchableOpacity onPress={onPress}>
        <Image
          source={{
            ...
            // uri: `http://api.adorable.io/avatar/${name}`,
          }}
          style={styles.avatar}
        />
      </TouchableOpacity>
...
const LocalAPI = () => {
  ...
  const getData = () => {
    Axios.get('http://10.0.2.2:3004/users').then((result) => {
      console.log('result get data:', result), setUsers(result.data);
    });
  };
  ...
  const deleteItem = (item) => {
    ...
    Axios.delete(`http://10.0.2.2:3004/users/${item.id}`).then(
      (result) => console.log('result delete:', result),
      setName(''),
      setEmail(''),
      setBidang(''),
      getData(),
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.textTitle}>Masukkan Anggota </Text>
      <TextInput
...
const styles = StyleSheet.create({
  ...
  textTitle: {textAlign: 'center', paddingVertical: 20},
